...
export default function MarxismLearnPage() {
  ...
  const totalTopics = marxismModules.reduce((sum, m) => sum + m.topics.length, 0);
  const totalQuizzes = marxismModules.reduce((sum, m) => sum + m.topics.reduce((s, t) => s + t.quizzes.length, 0), 0);

  return (
    <div>
      <div className="mb-8 animate-in">
        <h1 className="text-2xl font-bold text-[var(--text)] tracking-tight">马克思主义基本原理</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1">系统学习马克思主义三大组成部分：哲学、政治经济学、科学社会主义</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 animate-in" style={{ animationDelay: '0.06s' }}>
        ...
        <div className="card px-5 py-4 text-center">
          <div className="text-2xl font-bold" style={{ color: 'var(--success)' }}>{marxismModules.length}</div>
          <div className="text-[13px] text-[var(--text-muted)]">课程模块</div>
        </div>
        <div className="card px-5 py-4 text-center">

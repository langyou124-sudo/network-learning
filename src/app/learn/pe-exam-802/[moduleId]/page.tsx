...
export default function PeExam802ModulePage() {
  ...
  useEffect(() => {
    ...
    setModule(mod || null);
    if (mod) setTopics(mod.topics);
  }, [moduleId]);

  if (!module) {
    return <div className="text-center py-16"><p className="text-[var(--text-muted)]">模块不存在</p></div>;
  }

  return (
    <div>
      <div className="mb-8 animate-in">
        <Link href="/learn/pe-exam-802" className="text-[13px] text-[var(--accent)] hover:underline">← 返回802经济学综合</Link>
        <div className="flex items-center gap-3 mt-3">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl" style={{ background: 'var(--accent-light)' }}>
            {module.icon}
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8 animate-in" style={{ animationDelay: '0.06s' }}>
        ...
        <div className="card px-5 py-4 text-center">
          <div className="text-2xl font-bold text-[var(--success)]">{topics.reduce((sum, t) => sum + t.quizzes.length, 0)}</div>
      ...
      {topics.length > 0 ? (
        <div className="space-y-3">
          {topics.map((topic, index) => (
            <Link
              key={topic.id}
              href={`/learn/pe-exam-802/${moduleId}/topic/${topic.id}`}
            ...
            >
              <div className="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-[14.5px] font-semibold text-[var(--text)] line-clamp-1">{topic.title}</h3>
                  <p className="text-[13px] text-[var(--text-muted)] mt-0.5 line-clamp-1">{topic.description}</p>
                </div>
              </div>

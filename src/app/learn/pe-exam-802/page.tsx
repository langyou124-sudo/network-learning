...
export default function PeExam802Page() {
  ...
  const totalTopics = peExam802Modules.reduce((sum, m) => sum + m.topics.length, 0);
  const totalQuizzes = peExam802Modules.reduce((sum, m) => sum + m.topics.reduce((s, t) => s + t.quizzes.length, 0), 0);

  return (
    <div>
      <div className="mb-8 animate-in">
        <h1 className="text-2xl font-bold text-[var(--text)] tracking-tight">802经济学综合</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1">政治经济学(50分) + 微观经济学(50分) + 宏观经济学(50分)</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 animate-in" style={{ animationDelay: '0.06s' }}>
        ...
        <div className="card px-5 py-4 text-center">
          <div className="text-2xl font-bold" style={{ color: 'var(--success)' }}>{peExam802Modules.length}</div>
      ...
      <div className="space-y-5">
        {peExam802Modules.map((mod, index) => (
          <Link
            key={mod.id}
            href={`/learn/pe-exam-802/${mod.id}`}
